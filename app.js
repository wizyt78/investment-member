...
memberHighlights:"أعضاء مميزون",topMembers:"أعضاء مميزون",verifiedResults:"نتائج موثقة",ownerRole:"المالك",managerRole:"المدير",memberRole:"عضو",profileEarnings:"الأرباح",replaceProfilePhotos:"استبدل ملفات الصور في مجلد profiles مع الحفاظ على أسماء الملفات.",menu:"القائمة",theme:"المظهر",saved:"تم الحفظ.",statusCompleted:"مكتمل",statusApproved:"معتمد",statusPending:"قيد المراجعة",statusRejected:"مرفوض",statusActive:"نشط",
...
$("avatarFile").onchange=e=>{
 ...
 if(file.size>5*1024*1024){toast(lang==="ar"?"اختر صورة أصغر من 5MB.":"Choose an image under 5MB.");e.target.value="";return}
...
$("saveAvatar").onclick=async()=>{
 const b=$("saveAvatar");if(!pendingAvatar||!data)return;
 try{
  b.classList.add("busy");
  await api("/api/profile",{method:"PATCH",body:JSON.stringify({full_name:data.user.full_name||"",email:data.user.email||"",phone:data.user.phone||"",avatar_url:pendingAvatar})});
  data.user.avatar_url=pendingAvatar;
  try{localStorage.setItem(avatarKey(data.user.id),pendingAvatar)}catch{}
  pendingAvatar="";
  setAllAvatars();
  b.disabled=true;
  toast(T[lang].photoSaved);
 }catch(e){toast(e.message)}finally{b.classList.remove("busy")}
};
$("removeAvatar").onclick=async()=>{
 const b=$("removeAvatar");
 try{
  b.classList.add("busy");
  await api("/api/profile",{method:"PATCH",body:JSON.stringify({full_name:data.user.full_name||"",email:data.user.email||"",phone:data.user.phone||"",avatar_url:""})});
  data.user.avatar_url="";
  pendingAvatar="";
  try{localStorage.removeItem(avatarKey(data.user.id))}catch{}
  setAllAvatars();
  $("saveAvatar").disabled=true;
  toast(T[lang].photoRemoved);
 }catch(e){toast(e.message)}finally{b.classList.remove("busy")}
};
