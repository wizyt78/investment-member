...
$("loginForm").onsubmit=async e=>{e.preventDefault();const b=$("loginBtn");try{b.classList.add("busy");const d=await api("/api/auth/login",{method:"POST",body:JSON.stringify({username:$("loginUser").value.trim(),password:$("loginPass").value})});token=d.session;localStorage.setItem("ip_token",token);await load()}catch(e){toast(e.message)}finally{b.classList.remove("busy")}};
$("signupForm").onsubmit=async e=>{e.preventDefault();try{if($("signupPass").value!==$("signupPass2").value)throw Error(T[lang].badPassword);const d=await api("/api/auth/signup",{method:"POST",body:JSON.stringify({full_name:$("fullName").value.trim(),email:$("signupEmail").value.trim(),username:$("signupUser").value.trim(),phone:$("phone").value.trim(),password:$("signupPass").value})});token=d.session;localStorage.setItem("ip_token",token);await load()}catch(e){toast(e.message)}};
