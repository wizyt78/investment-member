...
function setAllAvatars(){
 ...
 $("profileName").textContent=name;$("profileHandle").textContent="@"+(data?.user?.username||"member");$("mobileWelcome").textContent=(lang==="ar"?"مرحباً، ":"Welcome, ")+name;
}
...
function render(){
 const w=data.wallet||{};
 $("welcome").textContent=(lang==="ar"?"مرحباً، ":"Welcome, ")+data.user.full_name;
 ...
 data.transactions.map(t=>`<div class="row"><span>${esc(t.description||t.type)}</span><b>${fmt(t.amount)}</b><span class="tag ${statusClass(t.status)}">${esc(t.status)}</span><span>${date(t.created_at)}</span></div>`).join(""):`<div class="empty">${T[lang].noTransactions}</div>`;
 $("investments").innerHTML=(data.investments||[]).length?data.investments.map(i=>`<div class="investment-item"><b>${esc(i.plan_name)}</b><div class="investment-meta"><span>${fmt(i.amount)}</span><span class="tag ${statusClass(i.status)}">${esc(i.status)}</span></div></div>`).join(""):`<div class="empty">${T[lang].noInvestments}</div>`;
 $("fundList").innerHTML=(data.funding_requests||[]).length?data.funding_requests.map(f=>`<div class="request-item"><b>${fmt(f.amount)}</b><div class="investment-meta"><span>${esc(f.payment_method)}</span><span class="tag ${statusClass(f.status)}">${esc(f.status)}</span></div><div class="investment-meta"><span>${date(f.created_at)}</span></div></div>`).join(""):`<div class="empty">${T[lang].noFunding}</div>`;
 $("withList").innerHTML=(data.withdrawal_requests||[]).length?data.withdrawal_requests.map(f=>`<div class="request-item"><b>${fmt(f.amount)}</b><div class="investment-meta"><span>${esc(f.method)}</span><span class="tag ${statusClass(f.status)}">${esc(f.status)}</span></div><div class="investment-meta"><span>${date(f.created_at)}</span></div></div>`).join(""):`<div class="empty">${T[lang].noWithdrawals}</div>`;
...
$("fundForm").onsubmit=async e=>{e.preventDefault();const b=$("fundBtn");try{b.classList.add("busy");await api("/api/funding",{method:"POST",body:JSON.stringify({amount:+$("fundAmount").value,payment_method:$("fundMethod").value.trim(),reference:$("fundRef").value.trim(),notes:$("fundNotes").value.trim()})});toast(T[lang].requestSent);$("fundForm").reset();await load();show("funding")}catch(e){toast(e.message)}finally{b.classList.remove("busy")}};
$("withdrawForm").onsubmit=async e=>{e.preventDefault();const b=$("withBtn");try{b.classList.add("busy");await api("/api/withdrawals",{method:"POST",body:JSON.stringify({amount:+$("withAmount").value,method:$("withMethod").value.trim(),destination:$("withDest").value.trim(),notes:$("withNotes").value.trim()})});toast(T[lang].requestSent);$("withdrawForm").reset();await load();show("withdraw")}catch(e){toast(e.message)}finally{b.classList.remove("busy")}};
